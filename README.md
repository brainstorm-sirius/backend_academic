...
Поддерживаемые названия столбцов (русский/английский):
...
- Таблицы создаются автоматически при первом запуске через `models.Base.metadata.create_all()`
...
├── model/                 # ML модель (не в git)
├── import_csv.py         # Скрипт импорта CSV данных
├── train_model.py        # Скрипт обучения ML модели
